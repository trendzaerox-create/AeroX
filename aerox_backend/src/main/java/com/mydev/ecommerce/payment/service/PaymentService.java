...
//     /**
//      * Guest Magic Checkout create-order path.
//      *
//      * Important security property: amount, title and stock are NEVER trusted from
//      * browser/localStorage. Only productId + quantity are accepted, and the server
//      * rebuilds the order from PostgreSQL.
//      */
//     public CreateRazorpayOrderResponse createGuestMagicCheckoutOrder(
//             CreateMagicCheckoutOrderRequest request
//     ) {
//         if (request == null || request.items() == null || request.items().isEmpty()) {
//             throw new RuntimeException("Cart is empty");
//         }

//         // Merge duplicate product ids while preserving cart order.
//         Map<Long, Integer> requestedQuantities = new LinkedHashMap<>();

//         for (CreateMagicCheckoutOrderRequest.Item requestedItem : request.items()) {
//             if (requestedItem == null
//                     || requestedItem.productId() == null
...
//             BigDecimal unitPrice = product.getPriceInr();

//             if (unitPrice == null || unitPrice.compareTo(BigDecimal.ZERO) <= 0) {
//                 throw new RuntimeException(
//                         "Product price is missing for: " + product.getTitle()
//                 );
//             }
...
//             // Required by Razorpay Magic Checkout. Without line_items_total,
//             // Razorpay can treat the order as Standard Checkout.
//             options.put("line_items_total", amountPaise);

//             JSONArray lineItems = new JSONArray();

//             for (OrderItem item : savedOrder.getItems()) {
//                 long unitPricePaise = toPaise(item.getUnitPrice());

//                 JSONObject lineItem = new JSONObject();
//                 String productId = String.valueOf(item.getProduct().getId());

//                 lineItem.put("sku", "PRODUCT-" + productId);
//                 lineItem.put("variant_id", productId);
//                 lineItem.put("price", unitPricePaise);
//                 lineItem.put("offer_price", unitPricePaise);
//                 lineItem.put("quantity", item.getQuantity());
//                 lineItem.put("name", item.getProductTitle());
//                 lineItem.put("description", item.getProductTitle());

//                 lineItems.put(lineItem);
//             }

//             options.put("line_items", lineItems);

//             JSONObject notes = new JSONObject();
//             notes.put("local_order_id", String.valueOf(savedOrder.getId()));
//             notes.put("order_number", savedOrder.getOrderNumber());
//             notes.put("checkout_mode", "MAGIC_GUEST");
//             options.put("notes", notes);
...
//             log.info(
//                     "Guest Magic Checkout order created -> localOrderId={}, orderNumber={}, razorpayOrderId={}, amountPaise={}",
//                     savedOrder.getId(),
//                     savedOrder.getOrderNumber(),
//                     razorpayOrderId,
//                     amountPaise
...
//         } catch (Exception exception) {
//             log.error(
//                     "Guest Magic Checkout order creation failed -> localOrderId={}, orderNumber={}, reason={}",
//                     savedOrder.getId(),
//                     savedOrder.getOrderNumber(),
//                     exception.getMessage(),
//                     exception
...
//             throw new RuntimeException(
//                     "Failed to create Magic Checkout order. Please try again.",
//                     exception
//             );
//         }
//     }

//     public VerifyRazorpayPaymentResponse verifyGuestMagicCheckoutPayment(
//             VerifyRazorpayPaymentRequest request
//     ) {
//         Order order = orderRepository
//                 .findLockedById(request.orderId())
//                 .orElseThrow(() -> new RuntimeException("Order not found"));

//         if (!"MAGIC".equalsIgnoreCase(order.getCheckoutMode())) {
//             throw new RuntimeException("This is not a Magic Checkout order");
//         }
...
//         // Fetch the Magic Checkout order back from Razorpay and persist the
//         // customer-selected shipping address before confirming local fulfilment.
//         hydrateMagicCheckoutCustomer(order);
...
//     public MagicCheckoutShippingInfoResponse getMagicCheckoutShippingInfo(
//             MagicCheckoutShippingInfoRequest request
//     ) {
//         if (request == null || !hasText(request.razorpayOrderId())) {
//             throw new RuntimeException("Razorpay order id is required");
//         }

//         String suppliedRazorpayOrderId = request.razorpayOrderId().trim();

//         Order order = orderRepository
//                 .findByRazorpayOrderId(suppliedRazorpayOrderId)
//                 .orElseGet(() -> {
//                     if (!suppliedRazorpayOrderId.startsWith("order_")) {
//                         return orderRepository
//                                 .findByRazorpayOrderId("order_" + suppliedRazorpayOrderId)
//                                 .orElse(null);
//                     }
//                     return null;
//                 });

//         if (order == null || !"MAGIC".equalsIgnoreCase(order.getCheckoutMode())) {
//             throw new RuntimeException("Magic Checkout order not found");
//         }

//         if (hasText(request.orderId())
//                 && !Objects.equals(order.getOrderNumber(), request.orderId())) {
//             throw new RuntimeException("Magic Checkout receipt mismatch");
//         }

//         List<MagicCheckoutShippingInfoResponse.Address> addresses =
//                 request.addresses() == null
//                         ? List.of()
//                         : request.addresses().stream().map(address -> {
//                             String country = hasText(address.country())
//                                     ? address.country().trim().toUpperCase(Locale.ROOT)
//                                     : "IN";

//                             String zipcode = hasText(address.zipcode())
//                                     ? address.zipcode().trim()
//                                     : "";

//                             // Matches the site's existing free-shipping behavior.
//                             // Replace this syntactic check with your courier/Shiprocket
//                             // serviceability API if you do not ship to every Indian PIN.
//                             boolean serviceable =
//                                     "IN".equals(country)
//                                             && zipcode.matches("^[1-9][0-9]{5}$");

//                             MagicCheckoutShippingInfoResponse.ShippingMethod method =
//                                     new MagicCheckoutShippingInfoResponse.ShippingMethod(
//                                             "FREE_STANDARD",
...
//                             return new MagicCheckoutShippingInfoResponse.Address(
//                                     address.id(),
//                                     zipcode,
//                                     country,
//                                     List.of(method)
...
//         return new MagicCheckoutShippingInfoResponse(addresses);
//     }
...
//         hydrateMagicCheckoutCustomerIfRequired(order);
...
//         return OrderEmailPayload.builder()
//                 .customerName(user != null ? user.getName() : order.getAddressFullName())
//                 .customerEmail(user != null ? user.getEmail() : order.getCustomerEmail())
//                 .orderNumber(order.getOrderNumber())
//                 .orderStatus(order.getStatus().name())
//                 .paymentMethod(order.getPaymentMethod().name())
//                 .paymentStatus(order.getPaymentStatus().name())
...
//     private void hydrateMagicCheckoutCustomerIfRequired(Order order) {
//         if (order != null && "MAGIC".equalsIgnoreCase(order.getCheckoutMode())) {
//             hydrateMagicCheckoutCustomer(order);
//         }
//     }
...
//     /**
//      * Reads customer_details.shipping_address from Razorpay's Magic Checkout
//      * order. We intentionally fail confirmation when a shipping address is still
//      * unavailable; Razorpay will retry a failing webhook and the browser verify
//      * endpoint can also retry. This is safer than marking an unfulfillable order
//      * CONFIRMED.
//      */
//     private void hydrateMagicCheckoutCustomer(Order order) {
//         try {
//             com.razorpay.Order remoteOrder =
//                     getRazorpayClient().orders.fetch(order.getRazorpayOrderId());

//             JSONObject remoteJson = new JSONObject(remoteOrder.toString());
//             JSONObject customerDetails = remoteJson.optJSONObject("customer_details");

//             if (customerDetails == null) {
//                 throw new RuntimeException("Magic Checkout customer details are not available yet");
//             }

//             JSONObject shippingAddress =
//                     customerDetails.optJSONObject("shipping_address");

//             if (shippingAddress == null) {
//                 throw new RuntimeException("Magic Checkout shipping address is not available yet");
//             }

//             String fullName = jsonText(shippingAddress, "name");
//             String phone = firstText(
//                     jsonText(shippingAddress, "contact"),
//                     jsonText(customerDetails, "contact")
//             );
//             String line1 = jsonText(shippingAddress, "line1");
//             String line2 = jsonText(shippingAddress, "line2");
//             String city = jsonText(shippingAddress, "city");
//             String state = jsonText(shippingAddress, "state");
//             String pincode = jsonText(shippingAddress, "zipcode");
//             String country = normaliseCountry(jsonText(shippingAddress, "country"));
//             String email = jsonText(customerDetails, "email");
...
//                     || !hasText(pincode)
//                     || !hasText(country)) {
//                 throw new RuntimeException("Magic Checkout returned an incomplete shipping address");
//             }

//             order.setCustomerEmail(email);
//             order.setAddressFullName(fullName);
//             order.setAddressPhone(phone);
//             order.setAddressLine1(line1);
//             order.setAddressLine2(line2);
//             order.setAddressCity(city);
//             order.setAddressState(state);
//             order.setAddressPincode(pincode);
//             order.setAddressCountry(country);

//             orderRepository.save(order);

//             log.info(
//                     "Magic Checkout customer/address saved -> orderNumber={}, emailPresent={}, pincode={}",
//                     order.getOrderNumber(),
//                     hasText(email),
//                     pincode
//             );
...
//         } catch (Exception exception) {
//             throw new RuntimeException(
//                     "Unable to read Magic Checkout customer/address from Razorpay",
//                     exception
//             );
//         }
//     }
...
public class PaymentService {
    ...
    private static final String INR = "INR";
    private static final int RAZORPAY_LINE_ITEM_NAME_MAX_LENGTH = 128;
    ...
    public CreateRazorpayOrderResponse createGuestMagicCheckoutOrder(
        ...
        for (Map.Entry<Long, Integer> entry : requestedQuantities.entrySet()) {
            ...
            Integer rawPrice = product.getPriceInr();

            if (rawPrice == null || rawPrice <= 0) {
                throw new RuntimeException(
                        "Product price is missing or invalid for: " + product.getTitle()
                );
            }

            BigDecimal unitPrice = BigDecimal.valueOf(rawPrice.longValue());
        ...
        try {
            ...
            for (OrderItem item : savedOrder.getItems()) {
                ...
                String safeProductName =
                        truncateRazorpayText(
                                item.getProductTitle(),
                                RAZORPAY_LINE_ITEM_NAME_MAX_LENGTH,
                                "Product " + productId
                        );
                ...
                lineItem.put("offer_price", unitPricePaise);
                lineItem.put("quantity", item.getQuantity());
                lineItem.put("name", safeProductName);
                lineItem.put("description", safeProductName);
    ...
    /**
     * Razorpay rejects Magic Checkout line-item names longer than 128 characters.
     * Keep the complete product title in our database/order snapshot, but send a
     * safe display value to Razorpay.
     */
    private String truncateRazorpayText(
            String value,
            int maxLength,
            String fallback
    ) {
        ...
        // Collapse new lines, tabs and repeated spaces into a single space.
