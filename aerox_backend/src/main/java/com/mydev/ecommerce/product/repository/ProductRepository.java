...
public interface ProductRepository extends JpaRepository<Product, Long> {
     ...
     * WishlistService can continue calling findByIdWithImages(id).
     */
    @Query("""
        SELECT DISTINCT p
        FROM Product p
    ...
    /*
     * Returns all publicly selectable colours for one product family.
     * Images are fetched because the PDP colour selector displays a thumbnail.
     */
    @Query("""
        SELECT DISTINCT p
        FROM Product p
        LEFT JOIN FETCH p.category
        LEFT JOIN FETCH p.images
        WHERE p.variantGroupCode = :variantGroupCode
          AND p.active = true
          AND p.deleted = false
        ORDER BY p.variantDisplayOrder ASC, p.displayOrder ASC, p.id ASC
    """)
    List<Product> findActiveColorVariantsByGroupCode(
            @Param("variantGroupCode") String variantGroupCode
    );
