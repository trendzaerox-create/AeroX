...
import CreateProductVariantSetup, {
  EMPTY_CREATE_VARIANT_SETUP,
  createVariantLinkPayload,
  validateCreateVariantSetup,
} from "@/components/admin/products/CreateProductVariantSetup";
import {
  getApiErrorMessage,
...
export default function CreateProductPage() {
  ...
  const [variantSetup, setVariantSetup] = useState(EMPTY_CREATE_VARIANT_SETUP);
  ...
  const handleCreate = async () => {
    ...
    const variantValidationError = validateCreateVariantSetup(
      variantSetup,
      categoryId
    );
    ...
    try {
      ...
      const createResponse = await api.post("/api/admin/products", data);
      ...
      if (!Number.isFinite(createdProductId) || createdProductId <= 0) {
        throw new Error("Product was created but the response did not include its ID");
      }

      if (variantSetup.enabled) {
        try {
          await linkProductColorVariant(
            createdProductId,
            createVariantLinkPayload(variantSetup)
          );
        } catch (variantError) {
          ...
          alert(
            `Product #${createdProductId} was created, but colour linking failed: ${getApiErrorMessage(
              variantError,
              "Unable to link the colour variant"
            )}`
          );

          router.push(`/admin/products/edit/${createdProductId}#colour-variants`);
          return;
        }
      }
      ...
      dispatch(clearImages());
      setPdpBannerImages([]);
      setVariantSetup(EMPTY_CREATE_VARIANT_SETUP);
      alert(
        variantSetup.enabled
          ? "Product created and linked to the colour group"
          : "Product created"
      );
      router.push(
        variantSetup.enabled
          ? `/admin/products/edit/${createdProductId}#colour-variants`
          : "/admin/products"
      );
    } catch (err) {
      console.error(err);
