...
import ColorVariantManager from "@/components/admin/products/ColorVariantManager";
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        ...
        setDiscountInr(
          p.discountInr === null || p.discountInr === undefined
            ? ""
            : String(p.discountInr),
        );
  ...
  const handleVariantProductChange = (updatedProduct) => {
    ...
    setProduct((current) => ({
      ...
      reviews: updatedProduct.reviews ?? current?.reviews ?? [],
  ...
  const handleUpdate = async () => {
    ...
    if (Number(price) > Number(discountInr)) {
      alert("Final selling price cannot be greater than discounted price");
      return;
    }
    ...
    >
      ...
      >
        ...
        <div style={{ display: "grid", gap: "18px" }}>
          ...
          <ColorVariantManager
            product={product}
            onProductChange={handleVariantProductChange}
          />
