...
async function getProducts() {
  ...
  if (!apiBase) {
    console.error("Product API URL is not configured");
    return [];
  }
  ...
  try {
    ...
    while (hasMorePages) {
      const url =
        `${normalizedApiBase}/api/products` +
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Failed to fetch products: ${res.status} ` +
            `${res.statusText} ${responseBody}`
        );
      }

      const data = await res.json();
      const currentProducts = extractProducts(data);
      ...
      } else if (Number.isInteger(data?.totalPages)) {
        hasMorePages = page + 1 < data.totalPages;
      } else {
        hasMorePages = currentProducts.length === PAGE_SIZE;
      }
...
export default async function BestSellersPage() {
  ...
  // Show every product returned by the API.
