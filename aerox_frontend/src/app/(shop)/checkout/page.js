...
// const forgotPasswordSuccessStyle = {
//   marginTop: 8,
//   marginBottom: 0,
//   color: "#166534",
//   fontSize: 13,
...
import {
  createRazorpayOrderApi,
  verifyRazorpayPaymentApi,
  createMagicCheckoutOrderApi,
  verifyMagicCheckoutPaymentApi,
} from "@/features/payment/paymentApi";

import { applyCouponApi } from "@/features/coupons/couponApi";
import { loadRazorpayScript } from "@/lib/loadRazorpay";
import { loadRazorpayMagicScript } from "@/lib/loadRazorpayMagic";
...
export default function CheckoutPage() {
  ...
  const pageDescription = isGiftSetMode
    ...
    : token
      ? "Complete your order by selecting a delivery address, reviewing your cart, and choosing your preferred payment method."
      : "Pay without creating an account. Razorpay Magic Checkout securely collects your contact and delivery address.";
  ...
  const handleGuestMagicPayment = async () => {
    setSuccessMessage("");

    if (token) {
      // Signed-in checkout deliberately keeps your existing COD/coupon/address
      // behavior unchanged.
      return handleOnlinePayment();
    }

    if (isGiftSetMode) {
      alert("Guest Magic Checkout is enabled for the regular product cart only.");
      return;
    }
    ...
    if (items.some((item) => !item.productId || item.quantity <= 0)) {
      alert("Cart contains an invalid item. Please refresh your cart.");
      return;
    }

    const loaded = await loadRazorpayMagicScript();

    if (!loaded) {
      alert("Razorpay Magic Checkout SDK failed to load");
      return;
    }
    ...
    try {
      ...
      const razorpayOrder = await createMagicCheckoutOrderApi({ items });
      ...
      const options = {
        ...
        // Your existing coupon rules are user-based. Do not expose Razorpay's
        // separate Magic coupon UI until promotion APIs are wired to the same rules.
        show_coupons: false,

        notes: {
          internalOrderId: String(razorpayOrder.orderId),
          source: "cart",
          checkoutMode: "MAGIC_GUEST",
        },

        handler: async function (response) {
          try {
            const verifyResponse = await verifyMagicCheckoutPaymentApi({
              orderId: razorpayOrder.orderId,
              razorpayOrderId: response.razorpay_order_id,
              razorpayPaymentId: response.razorpay_payment_id,
              razorpaySignature: response.razorpay_signature,
            ...
            // clearCart already clears localStorage guest cart when no JWT exists.
    ...
    <>
      <div className="checkout-shell">
        ...
        <div className="checkout-container">
          ...
          <div className="checkout-layout">
            <div className="left-column">
              {checkoutStep === "email" && !token && !isGiftSetMode && (
                <section className="glass-card fade-up delay-1 account-checkout-card">
                  <div className="section-head">
                    <div>
                      <p className="section-kicker">Guest Checkout</p>
                      <h2 className="section-title">No account required</h2>
                      <p className="section-desc">
                        Continue with Razorpay Magic Checkout. Razorpay will ask
                        for your mobile number, email and delivery address inside
                        the secure checkout. Your app will not create a customer
                        login.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="inner-panel">
                    <div className="payment-title">Online payment</div>
                    <div className="payment-desc">
                      UPI, cards and other payment methods enabled on your
                      Razorpay account. Shipping remains free, matching your
                  ...
                  <button
                    type="button"
                    onClick={handleGuestMagicPayment}
                    disabled={disablePlaceOrder}
                    className={`place-order-btn ${
                      disablePlaceOrder ? "disabled" : ""
                    }`}
                  >
                    {processingOnlinePayment
                      ...
                      : `Pay ₹${finalTotal} with Magic Checkout`}
                  </button>
                </section>
              )}
