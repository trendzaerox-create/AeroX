...
export default function ProductPage() {
  ...
  /*
   * Warm only the neighbouring gallery images through Next.js' optimizer.
   * The visible image is already requested with priority below. Preloading the
   * whole gallery would compete with it and make the first render slower.
   */
  useEffect(() => {
    if (!galleryMedia.length || isActiveVideo) return;

    const neighbourIndexes = [activeMediaIndex - 1, activeMediaIndex + 1];
  ...
  }, [activeMediaIndex, galleryMedia, isActiveVideo]);
                    ...
                    >
                      {isActiveVideo ? (
                        ...
                        <Image
                          ...
                          width={1200}
                          height={1200}
                          sizes="(max-width: 767px) calc(100vw - 40px), (max-width: 1199px) 58vw, 680px"
                          quality={75}
                          priority
                          draggable={false}
                          style={{
