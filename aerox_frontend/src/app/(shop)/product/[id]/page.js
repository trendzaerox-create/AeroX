...
function getDisplayValue(item) {
  ...
  const name =
    item.name || item.item || item.label || item.title || item.value || "";
...
export default function ProductPage() {
  ...
  // ---------------------------------------------------------------------
  // Perf note: everything below used to be plain `const` computations that
  // lived AFTER the "if (!product) return <Loading/>" check. That meant
  // JSON.parse + array map/filter/sort work for specs, features, FAQs,
  // box contents, compatibility, banners, and color variants ran on EVERY
  // single re-render of this component -- including every pixel of image
  // zoom-dragging, every keystroke in the review textarea, and every FAQ
  // toggle, since none of those interactions actually change `product`.
  // Wrapping them in useMemo (keyed on the relevant product fields) means
  // this work now only runs when the product itself changes, which is the
  // main fix for the slow/janky page feel. Hooks must be called
  // unconditionally, so these are now defined above the early return, and
  // use optional chaining to stay safe while `product` is still loading.
  // ---------------------------------------------------------------------
  ...
  }, [product?.colorVariants, product?.id]);
  ...
  const roundedAverage = reviewCount > 0 ? Math.round(Number(averageRating)) : 0;

  const shortHighlights = useMemo(
    () => splitHighlights(product?.shortHighlights),
  ...
  const faqs = useMemo(() => {
    const rawFaqs = safeJsonParse(rawFaqsSource, []);

    return Array.isArray(rawFaqs)
      ? rawFaqs
          .map((item) => {
            if (!item) return null;
  ...
  const boxContents = useMemo(
    () => safeJsonParse(product?.boxContents || product?.boxContentsJson, []),
    [product?.boxContents, product?.boxContentsJson],
  );

  const visibleBoxContents = useMemo(
    () =>
      Array.isArray(boxContents)
  ...
  const { compatibilityIsList, compatibilityItems, hasCompatibility } =
    useMemo(() => {
      const compatibilityParsed = safeJsonParse(compatibilityRaw, null);

      const isList =
        Array.isArray(compatibilityParsed) ||
        String(compatibilityRaw).includes("|");

      const items = Array.isArray(compatibilityParsed)
      ...
      return {
        compatibilityIsList: isList,
        compatibilityItems: items,
        hasCompatibility: items.length > 0,
      };
    }, [compatibilityRaw]);

  const pdpBanners = useMemo(() => {
    const banners = safeJsonParse(
      product?.pdpBanners ||
        product?.pdpBannersJson ||
        product?.pdp_banners_json,
      [],
  ...
  return (
    <>
        ...
        <div style={{ maxWidth: "1380px", margin: "0 auto" }}>
          ...
          {pdpBanners.length > 0 && (
            <div className="pdp-banners-section">
              {pdpBanners.map((banner, index) => (
                <div
                  key={`${banner.imageUrl}-${index}`}
                  className="pdp-banner-card"
                >
                  <img
                ...
                <div className="faq-list">
                  {faqs.map((item, index) => {
                    ...
                    return (
                      <div
                        key={index}
                        className={`faq-item ${isFaqOpen ? "faq-open" : ""}`}
                      >
                        <button
                          type="button"
                          className="faq-summary"
                          onClick={() =>
                            setOpenFaqIndex(isFaqOpen ? null : index)
                          }
      ...
      <style jsx>{`
        ...
        .faq-item.faq-open .faq-arrow-wrap {
        ...
        .faq-answer {
          ...
          color: #64748b;
          line-height: 1.8;
          animation: faqAnswerSlide 0.24s ease both;
        }
        ...
        /* FINAL PRODUCT TITLE FONT SIZE OVERRIDE */
        .product-title {
...
//                               </div>
//                             ) : (
//                               // PERF: wrapper div gives next/image a sized,
//                               // position:relative box to fill. sizes is capped
//                               // small since thumbnails never render larger than
//                               // ~100px, so the CDN/optimizer serves a tiny file
//                               // instead of the full-resolution original.
//                               <span className="thumbnail-img-wrap">
//                                 <Image
//                                   src={media.url}
//                                   alt={`${product.title} thumbnail ${i + 1}`}
//                                   fill
...
//                         </video>
//                       ) : (
//                         // PERF: `priority` only on the very first image the
//                         // user sees (the LCP element) so it's preloaded and
//                         // NOT lazy-loaded. Every other gallery image the user
//                         // switches to lazy-loads its optimized size on demand
//                         // instead of all images downloading up front.
//                         <Image
//                           src={activeMedia.url}
//                           alt={activeMedia.alt || product.title}
//                           fill
...
//                     className="pdp-banner-card"
//                   >
//                     {/* PERF: banners are almost always below the fold, so
//                         they should never be part of the initial page weight.
//                         fill + lazy means the browser only fetches these once
...
//         /* PERF: wrapper that gives the next/image fill-mode thumbnail a
//            sized, relatively-positioned box (mirrors .thumbnail-video-wrap). */
//         .thumbnail-img-wrap {
//           position: relative;
//           display: block;
//           width: 100%;
