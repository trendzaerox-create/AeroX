...
export default function ProductCard({ product }) {
  const [loadSecondImage, setLoadSecondImage] = useState(false);
  ...
  return (
    ...
    >
      <article
        ...
        className="group flex flex-col overflow-hidden rounded-[10px] border border-neutral-200 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.10)] transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_10px_24px_rgba(0,0,0,0.16)] sm:h-full sm:rounded-[14px]"
      >
        {/* Product image */}
        <div className="relative w-full overflow-hidden rounded-t-[10px] bg-white sm:rounded-t-[14px]">
          <div className="relative aspect-square w-full">
            ...
            {firstImageUrl ? (
              <div className="relative h-full w-full overflow-hidden">
                ...
                {/* Second image loads only after hover/focus */}
                {secondImageUrl && loadSecondImage && (
                  <Image
                    src={secondImageUrl}
                    alt={`${
