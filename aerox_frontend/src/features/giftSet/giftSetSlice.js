...
const GUEST_GIFTSET_STORAGE_KEY =
  "guest_giftset";
...
function calculateSummary(items) {
  ...
  const discountAmountInr = Math.round(
    subtotal * (discountPercent / 100)
  );

  const finalTotalInr =
    subtotal - discountAmountInr;
...
/*
 * Fetch the authenticated server gift-set cart.
 *
 * Logged-out users must not call the protected
 * /api/giftset-cart endpoint. Their cart is loaded
 * from localStorage instead.
 */
export const fetchGiftSetCart =
  createAsyncThunk(
    "giftSet/fetchGiftSetCart",
    async (_, thunkAPI) => {
      ...
      try {
        ...
        /*
         * If the stored token is no longer accepted,
         * show the local guest cart instead of leaving
         * the gift-set state empty.
         */
        if (
          status === 401 ||
          status === 403
...
export const addGiftSetCartItem =
  createAsyncThunk(
    "giftSet/addGiftSetCartItem",
    async (payload, thunkAPI) => {
      try {
        return await giftSetApi.addGiftSetCartItem(
          payload
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
...
export const removeGiftSetCartItem =
  createAsyncThunk(
    "giftSet/removeGiftSetCartItem",
    async (itemId, thunkAPI) => {
      try {
        return await giftSetApi.removeGiftSetCartItem(
          itemId
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
...
export const placeGiftSetOrder =
  createAsyncThunk(
    "giftSet/placeGiftSetOrder",
    async (payload, thunkAPI) => {
      try {
        return await giftSetApi.placeGiftSetOrder(
          payload
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
          message:
            error?.response?.data?.message ||
            "Failed to place gift set order",
          status:
            error?.response?.status || null,
        });
      }
...
export const createGiftSetRazorpayOrder =
  createAsyncThunk(
    "giftSet/createGiftSetRazorpayOrder",
    async (payload, thunkAPI) => {
      try {
        return await giftSetApi.createGiftSetRazorpayOrder(
          payload
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
...
export const verifyGiftSetRazorpayPayment =
  createAsyncThunk(
    "giftSet/verifyGiftSetRazorpayPayment",
    async (payload, thunkAPI) => {
      try {
        return await giftSetApi.verifyGiftSetRazorpayPayment(
          payload
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
...
export const fetchMyGiftSetOrders =
  createAsyncThunk(
    "giftSet/fetchMyGiftSetOrders",
    async (_, thunkAPI) => {
      try {
        ...
        return thunkAPI.rejectWithValue({
          message:
            error?.response?.data?.message ||
            "Failed to fetch gift set orders",
          status:
            error?.response?.status || null,
        });
      }
...
export const fetchMyGiftSetOrderById =
  createAsyncThunk(
    "giftSet/fetchMyGiftSetOrderById",
    async (id, thunkAPI) => {
      try {
        return await giftSetApi.fetchMyGiftSetOrderById(
          id
        );
      } catch (error) {
        return thunkAPI.rejectWithValue({
          message:
            error?.response?.data?.message ||
            "Failed to fetch gift set order details",
          status:
            error?.response?.status || null,
        });
      }
...
export const syncGuestGiftSetCart =
  createAsyncThunk(
    "giftSet/syncGuestGiftSetCart",
    async (_, thunkAPI) => {
      const token = getToken();
      ...
      /*
       * Synchronisation is only possible after login.
       * This prevents accidental protected API calls
      ...
      try {
        ...
        const serverCart =
          await giftSetApi.getGiftSetCart();
...
const giftSetSlice = createSlice({
  ...
  reducers: {
    ...
    addGuestGiftSetItem(
      ...
      if (currentItems.length >= 5) {
        state.error =
          "Maximum 5 products allowed in gift set";
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        removeGiftSetCartItem.fulfilled,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        removeGiftSetCartItem.rejected,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        syncGuestGiftSetCart.fulfilled,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        createGiftSetRazorpayOrder.pending,
        (state) => {
          state.loading = true;
          state.error = null;
        }
      ...
      .addCase(
        createGiftSetRazorpayOrder.fulfilled,
        (state, action) => {
          state.loading = false;
          state.razorpayOrder =
            action.payload;
      ...
      .addCase(
        createGiftSetRazorpayOrder.rejected,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        verifyGiftSetRazorpayPayment.pending,
        (state) => {
          state.loading = true;
          state.error = null;
        }
      ...
      .addCase(
        verifyGiftSetRazorpayPayment.fulfilled,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        verifyGiftSetRazorpayPayment.rejected,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        fetchMyGiftSetOrders.fulfilled,
        (state, action) => {
          state.loading = false;
          state.orders =
            action.payload || [];
      ...
      .addCase(
        fetchMyGiftSetOrderById.pending,
        (state) => {
          state.loading = true;
          state.error = null;
          state.selectedOrder = null;
      ...
      .addCase(
        fetchMyGiftSetOrderById.fulfilled,
        (state, action) => {
          state.loading = false;
      ...
      .addCase(
        fetchMyGiftSetOrderById.rejected,
        (state, action) => {
          state.loading = false;
